...
class Application(object):
    ...
    def save_trac_token(self, token):
        self.config.token = token
        print('Saved trac token.')
    ...
    def print_config(self, ssh_keys=True):
        ...
        if token_based:
            ...
            print('Password: {0}'.format('*' * len(c.password)))
        else:
            print(dedent("""\
                Anonymous (read) access only. To configure an account, use:

                    git trac config --user=<name> --pass=<password>

                or use token-based authentication by logging into Trac and
                copying the generated token provided at
                ...
                Note: Token-based authentication is required if you log into
                Trac using GitHub.
                """.format(c.server_hostname)))

        if (token_based or password_based) and ssh_keys:
